...
// const urlObj =
//   `http://api.weatherapi.com/v1/current.json?key=${apiKey}&q=${searchItems}`;

// const urlObj2 =  `http://api.weatherapi.com/v1/forecast.json?key=${apiKey}&q=cameroon&days=6`
...
//     info.innerHTML +=  `${data.location.country}, ${data.location.region}`;

//     date.innerHTML = `${data.location.localtime};  ${data.location.
// tz_id
//     }`
...
//     <span> rosée <br> <b>${data.current.dewpoint_c}°C</b></span>
//     <span>vitesse du vent  <br><b>${data.current.wind_kph}kph</b></span>
//     <span>direction <br><b>${data.current.wind_dir}</b></span>
//     <span>dégré du vent <br><b>${data.current.wind_degree}°C</b></span>
//     <span>rafale: <br><b>${data.current.gust_kph}kph</b></span>
//     <span>pression <br><b>${data.current.pressure_mb}</b></span>
//     <span>couverture nuageuse <br><b>${data.current.cloud}%</b></span>`
...
//       <span>${date}</span>
//       <span>${condition}</span>
//       <span><img src="${icon}" style="width:30px"></span>
//       <span>${tempMax}°C</span>
//       <span>${tempMin}°C</span>
...
//       console.log(date, condition, icon, tempMax, tempMin, pluie)
//     });
...
function fetchCurrentWeather(searchItems) {
  const urlObj = `http://api.weatherapi.com/v1/current.json?key=${apiKey}&q=${encodeURIComponent(searchItems)}`;
...
async function fetchData(ville) {
  try {
    const urlObj2 = `http://api.weatherapi.com/v1/forecast.json?key=${apiKey}&q=${encodeURIComponent(ville)}&days=6`;
    const reponse = await fetch(urlObj2);
    const data = await reponse.json();
    const jour = data.forecast.forecastday;
